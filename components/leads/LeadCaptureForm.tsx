...
export function LeadCaptureForm({ formId }: LeadCaptureFormProps) {
    ...
    try {
      const response = await fetch('/api/submitLeadForm', {
        ...
        body: JSON.stringify({
          ...formData,
          form_id: formId,
          source: `sms_link_${formId}`, // Track that this came from SMS
        }),
      });
  ...
  return (
    <Card className="w-full max-w-md shadow-2xl">
      <CardHeader>
        <CardTitle className="text-2xl text-center">Get Your Free Home Valuation</CardTitle>
        <p className="text-sm text-gray-600 text-center dark:text-gray-400">Mike&apos;s Real Estate Team</p>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Select onValueChange={(value) => setFormData({...formData, timeline: value})} required>
